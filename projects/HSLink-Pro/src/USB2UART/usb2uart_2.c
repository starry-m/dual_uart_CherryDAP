...
#define UART_BASE                  HPM_UART1
#define UART_IRQ                   IRQn_UART1
#define UART_CLK_NAME              clock_uart1
#define UART_RX_DMA                HPM_DMA_SRC_UART1_RX
#define UART_RX_DMA_RESOURCE_INDEX (0U)
#define UART_RX_DMA_BUFFER_SIZE    (4096U)
#define UART_RX_DMA_BUFFER_COUNT   (3)

#define UART_TX_DMA                HPM_DMA_SRC_UART1_TX
...
ATTR_PLACE_AT_NONCACHEABLE_BSS_WITH_ALIGNMENT(4)
uint8_t uart_2nd_rx_buf[UART_RX_DMA_BUFFER_COUNT][UART_RX_DMA_BUFFER_SIZE];
ATTR_PLACE_AT_NONCACHEABLE_BSS_WITH_ALIGNMENT(8)
dma_linked_descriptor_t rx_2nd_descriptors[UART_RX_DMA_BUFFER_COUNT];

static dma_resource_t dma_2nd_resource_pools[2];
volatile uint32_t g_uart_2nd_tx_transfer_length = 0;

static hpm_stat_t board_uart_2nd_dma_config(void);
...
static void dma_channel_tc_callback(DMA_Type *ptr, uint32_t channel, void *user_data)
{
    ...
    (void) user_data;
    uint32_t i;
    dma_resource_t *rx_resource = &dma_2nd_resource_pools[UART_RX_DMA_RESOURCE_INDEX];
    dma_resource_t *tx_resource = &dma_2nd_resource_pools[UART_TX_DMA_RESOURCE_INDEX];
    uint32_t link_addr = ptr->CHCTRL[channel].LLPOINTER;
    uint32_t rx_desc_size = (sizeof(rx_2nd_descriptors) / sizeof(dma_linked_descriptor_t));
    if (rx_resource->channel == channel) {
        for (i = 0; i < rx_desc_size; i++) {
            if (link_addr == core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t) &rx_2nd_descriptors[i])) {
                chry_ringbuffer_write(&g_uartrx_2nd, &uart_2nd_rx_buf[i][rb_write_pos], UART_RX_DMA_BUFFER_SIZE - rb_write_pos);
                rb_write_pos = 0;
            }
        }
    }
    if (tx_resource->channel == channel) {
        chry_dap_usb2uart_2nd_uart_send_complete(g_uart_2nd_tx_transfer_length);
    }
}

void uart_2nd_isr(void)
{
    ...
    if (uart_is_rxline_idle(UART_BASE)) {
        ...
        uart_received_data_count =
                UART_RX_DMA_BUFFER_SIZE - dma_get_remaining_transfer_size(rx_resource->base, rx_resource->channel);
        if (uart_received_data_count > 0) {
            for (i = 0; i < rx_desc_size; i++) {
                if (link_addr == core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t) &rx_2nd_descriptors[i])) {
                    chry_ringbuffer_write(&g_uartrx_2nd, &uart_2nd_rx_buf[i][rb_write_pos],
...
SDK_DECLARE_EXT_ISR_M(UART_IRQ, uart_2nd_isr)

void usb2uart_2nd_handler(void)
{
    dma_resource_t *rx_resource = &dma_2nd_resource_pools[UART_RX_DMA_RESOURCE_INDEX];
    const uint32_t rx_desc_size = (sizeof(rx_2nd_descriptors) / sizeof(dma_linked_descriptor_t));
    uint32_t uart_received_data_count =
            UART_RX_DMA_BUFFER_SIZE - dma_get_remaining_transfer_size(rx_resource->base, rx_resource->channel);
...
static void uartx_2nd_io_init(void)
{
...
//    }
    HPM_IOC->PAD[PIN_UART_2ND_RX].FUNC_CTL = IOC_PA06_FUNC_CTL_UART1_RXD;
    HPM_IOC->PAD[PIN_UART_2ND_TX].FUNC_CTL = IOC_PA07_FUNC_CTL_UART1_TXD;
...
//
//    } else {
//        HPM_IOC->PAD[PIN_UART_2ND_RX].FUNC_CTL = IOC_PAD_FUNC_CTL_ALT_SELECT_SET(0);;
//        HPM_IOC->PAD[PIN_UART_2ND_TX].FUNC_CTL = IOC_PAD_FUNC_CTL_ALT_SELECT_SET(0);
//        gpiom_set_pin_controller(HPM_GPIOM, GPIO_GET_PORT_INDEX(PIN_UART_2ND_RX), GPIO_GET_PIN_INDEX(PIN_UART_2ND_RX),
//                                 gpiom_soc_gpio0);
//        gpio_set_pin_input(HPM_GPIO0, GPIO_GET_PORT_INDEX(PIN_UART_2ND_RX), GPIO_GET_PIN_INDEX(PIN_UART_2ND_RX)); // RX 默认输入
//
//        gpiom_set_pin_controller(HPM_GPIOM, GPIO_GET_PORT_INDEX(PIN_UART_2ND_TX), GPIO_GET_PIN_INDEX(PIN_UART_2ND_TX),
//                                 gpiom_soc_gpio0);
//        gpio_set_pin_output(HPM_GPIO0, GPIO_GET_PORT_INDEX(PIN_UART_2ND_TX), GPIO_GET_PIN_INDEX(PIN_UART_2ND_TX));
//
//        gpio_write_pin(HPM_GPIO0, GPIO_GET_PORT_INDEX(PIN_UART_2ND_TX), GPIO_GET_PIN_INDEX(PIN_UART_2ND_TX), 0);
//    }
...
void uartx_2nd_preinit(void)
{
    ...
    uart_clear_rxline_idle_flag(UART_BASE);
    if (board_uart_2nd_dma_config() != status_success) {
        return;
    }
}
...
void chry_dap_usb2uart_2nd_uart_send_bydma(uint8_t *data, uint16_t len)
{
    uint32_t buf_addr;
    dma_resource_t *tx_resource = &dma_2nd_resource_pools[UART_TX_DMA_RESOURCE_INDEX];
    if (len <= 0) {
    ...
    g_uart_2nd_tx_transfer_length = len;
...
static hpm_stat_t board_uart_2nd_dma_config(void)
{
    ...
    /* uart rx dma config */
    resource = &dma_2nd_resource_pools[UART_RX_DMA_RESOURCE_INDEX];
    if (dma_mgr_request_resource(resource) == status_success) {
        ...
        for (i = 0; i < rx_desc_size; i++) {
            if (i < (rx_desc_size - 1)) {
                chg_config.dst_addr = core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t) uart_2nd_rx_buf[i]);
                chg_config.linked_ptr = core_local_mem_to_sys_address(BOARD_RUNNING_CORE,
                                                                      (uint32_t) &rx_2nd_descriptors[i]);
            } else {
                chg_config.dst_addr = core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t) uart_2nd_rx_buf[0]);
                chg_config.linked_ptr = core_local_mem_to_sys_address(BOARD_RUNNING_CORE,
                                                                      (uint32_t) &rx_2nd_descriptors[0]);
            }
            if (dma_mgr_config_linked_descriptor(resource, &chg_config,
                                                 (dma_mgr_linked_descriptor_t *) &rx_2nd_descriptors[i]) !=
                status_success) {
                printf("generate dma desc fail\n");
                return status_fail;
            }
            rx_2nd_descriptors[i].ctrl &= ~DMA_MGR_INTERRUPT_MASK_TC;
            printf("linker_addr:0x%08x  %d\n", (uint32_t) &rx_2nd_descriptors[i], i);
        }
        chg_config.dst_addr = core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t) uart_2nd_rx_buf[0]);
        chg_config.linked_ptr = core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (uint32_t) &rx_2nd_descriptors[0]);
    ...
    /* uart tx dma config */
    resource = &dma_2nd_resource_pools[UART_TX_DMA_RESOURCE_INDEX];
    if (dma_mgr_request_resource(resource) == status_success) {
